...
abc.addEventListener('load', () => {
  if (abc.status == 200) {
    ...
    //Picking Patient Info
    document.getElementById('pickName').textContent = sessionStorage.getItem('chave')
...
function findAppointmentAtt(people) {
    return people
    .filter(function (p) {
      return p.patientId === parseInt(sessionStorage.getItem('id'))
    })
    .map(function (p) {
      return p
    })
